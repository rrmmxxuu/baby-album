...
export function SettingsBabiesScene({ className, authToken, albumOptions, onBack, onAdd, onOpenAlbumSettings }: SettingsBabiesSceneProps) {
  return (
    <article className={className}>
      ...
      <div className="stackList">
        {albumOptions.map((item) => (
          <SettingsListButton
            key={item.album.id}
            leading={<BabyAvatar albumId={item.album.id} baby={item.baby ?? null} className="settingsCardAvatar" token={authToken} />}
