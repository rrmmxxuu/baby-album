...
func (s *PostgresStore) AttachUploadContent(userID, sessionID string, input UploadContentInput) (domain.UploadSession, error) {
	...
	if _, err := tx.Exec(`update media_assets set byte_size = $1, original_blob_key = $2, content_sha256 = $3, media_type = coalesce(nullif($4::text, ''), media_type), captured_at = coalesce($5::timestamptz, captured_at), timeline_day = coalesce(nullif($6::text, ''), timeline_day), width = $7, height = $8, preview_status = $9, preview_blob_key = $10, screen_preview_status = $11, screen_preview_object_key = $12, original_local_state = 'online', original_restore_state = 'idle', original_last_accessed_at = $13 where id = $14`, input.ByteSize, input.BlobKey, strings.ToLower(strings.TrimSpace(input.ContentSHA256)), detectedMediaType, nullableTime(detectedCapturedAt), detectedTimelineDay, input.Width, input.Height, input.PreviewStatus, input.PreviewBlobKey, input.ScreenPreviewStatus, input.ScreenPreviewObjectKey, now, session.MediaID); err != nil {
		return domain.UploadSession{}, err
	}
