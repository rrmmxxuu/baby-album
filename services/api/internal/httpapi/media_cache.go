...
func (c *mediaCacheController) attachScreenPreviewFromBestSourceLocked(ctx context.Context, item domain.MediaAsset) (domain.MediaAsset, error) {
	...
	if item.OriginalR2State == "online" && strings.TrimSpace(item.OriginalR2Key) != "" {
		if updated, err := c.attachScreenPreviewFromWarmOriginalLocked(ctx, item); err == nil {
			return updated, nil
		}
	}
	return item, store.ErrNotFound
...
func (c *mediaCacheController) attachScreenPreviewFromWarmOriginalLocked(ctx context.Context, item domain.MediaAsset) (domain.MediaAsset, error) {
	result, err := c.OpenWarmOriginal(ctx, item)
	if err != nil {
		return item, err
	}
	defer result.Body.Close()
	...
	defer cleanup()
	return c.attachScreenPreviewFromSourcePathLocked(ctx, item, sourcePath, firstNonEmptyBlobKey(item.OriginalR2Key, item.ID))
}
