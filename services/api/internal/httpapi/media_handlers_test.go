...
const sequenceHEICBase64 = ""
...
func TestGenerateBestStillImagePreviewConvertsHEICSequence(t *testing.T) {
	if _, err := exec.LookPath("heif-convert"); err != nil {
		t.Skip("heif-convert is not available")
	}
	heicData, err := base64.StdEncoding.DecodeString(sequenceHEICBase64)
	if err != nil {
		t.Fatalf("decode sequence heic fixture: %v", err)
	}
	sourcePath := filepath.Join(t.TempDir(), "sequence.heic")
	if err := os.WriteFile(sourcePath, heicData, 0o644); err != nil {
		t.Fatalf("write sequence heic fixture: %v", err)
	}

	width, height, preview, err := generateBestStillImagePreview(sourcePath, "image/heic", "sequence.heic", 480, 82)
	if err != nil {
		t.Fatalf("generate heic sequence preview: %v", err)
	}
	if width <= 0 || height <= 0 {
		t.Fatalf("expected positive dimensions, got %dx%d", width, height)
	}
	maxRGB, err := maxRGBSum(preview)
	if err != nil {
		t.Fatalf("measure sequence preview brightness: %v", err)
	}
	if maxRGB < 10_000 {
		t.Fatalf("expected visible non-black sequence preview, max RGB sum was %d", maxRGB)
	}
}
